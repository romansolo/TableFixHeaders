...
/**
 * TableFixHeaders shows a table which can scroll in both directions. Also still
 * leaves the headers fixed.
 *
 * @author Brais Gabín (InQBarna)
 *
 *
 * NestedTableFixHeaders is port of TableFixHeaders, with supports acting
 * as both a nested scrolling parent and child on both new and old versions of Android.
 * Nested scrolling is enabled by default.
 *
 * @author Roman Solotopov <f4rw311@gmail.com>
 */
public class NestedTableFixHeaders extends ViewGroup implements NestedScrollingParent, NestedScrollingChild {
	private static final String TAG = "NestedTable";
	 ...
	 */
	public NestedTableFixHeaders(Context context) {
		this(context, null);
	}
	 ...
	 */
	public NestedTableFixHeaders(Context context, AttributeSet attrs) {
		super(context, attrs);
	...
	private void onSecondaryPointerUp(MotionEvent ev) {
		...
		if (pointerId == mActivePointerId) {
			...
			currentX = (int) MotionEventCompat.getY(ev, newPointerIndex);
			currentY = (int) MotionEventCompat.getY(ev, newPointerIndex);
	...
	public boolean onInterceptTouchEvent(MotionEvent event) {
		...
		switch (action & MotionEventCompat.ACTION_MASK) {
			case MotionEvent.ACTION_MOVE: {
				...
				final int xDiff = Math.abs(x - currentX);
				final int yDiff = Math.abs(y - currentY);
				Log.e(TAG, "axis: "+getNestedScrollAxes());
				if ((yDiff > touchSlop && (getNestedScrollAxes() & ViewCompat.SCROLL_AXIS_VERTICAL) == 0) ||
						(xDiff > touchSlop && (getNestedScrollAxes() & ViewCompat.SCROLL_AXIS_HORIZONTAL) == 0)) {
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		switch (actionMasked) {
			...
			case MotionEvent.ACTION_MOVE: {
				...
				final int y = (int) MotionEventCompat.getY(event, activePointerIndex);
				...
				boolean scrollLeft       = deltaX < 0 && getActualScrollX() > 0;
				boolean scrollRight      = deltaX > 0 && getActualScrollX() < getMaxScrollX();
				boolean scrollVertical   = Math.abs(deltaY) > touchSlop;
				boolean scrollHorizontal = Math.abs(deltaX) > touchSlop && (scrollLeft || scrollRight);

				if (!mIsBeingDragged && (scrollVertical || scrollHorizontal)) {
					final ViewParent parent = getParent();
					if (parent != null) {
						parent.requestDisallowInterceptTouchEvent(true);
					}
				...
				if (mIsBeingDragged) {
					currentX = x - mScrollOffset[0];
					currentY = y - mScrollOffset[1];
					...
					if (dispatchNestedScroll(scrolledDeltaX, scrolledDeltaY, unconsumedX, unconsumedY, mScrollOffset)) {
						currentX -= mScrollOffset[0];
						currentY -= mScrollOffset[1];
						vtev.offsetLocation(mScrollOffset[0], mScrollOffset[1]);
						mNestedXOffset += mScrollOffset[0];
						mNestedYOffset += mScrollOffset[1];
					}
			...
			case MotionEvent.ACTION_UP: {
				if (mIsBeingDragged) {
					...
					int initialXVelocity = (int) VelocityTrackerCompat.getXVelocity(tracker, mActivePointerId);
					int initialYVelocity = (int) VelocityTrackerCompat.getYVelocity(tracker, mActivePointerId);

					if ((Math.abs(initialXVelocity) > minimumVelocity) ||
							(Math.abs(initialYVelocity) > minimumVelocity)) {
						flingWithNestedDispatch(-initialXVelocity, -initialYVelocity);
						flinger.start(getActualScrollX(), getActualScrollY(), initialXVelocity, initialYVelocity, getMaxScrollX(), getMaxScrollY());
					}
				}
				mActivePointerId = INVALID_POINTER;
				endDrag();
			...
			case MotionEventCompat.ACTION_POINTER_DOWN: {
				...
				currentX = (int) MotionEventCompat.getX(event, index);
				currentY = (int) MotionEventCompat.getY(event, index);
				mActivePointerId = MotionEventCompat.getPointerId(event, index);
				break;
			}
			case MotionEventCompat.ACTION_POINTER_UP:
				onSecondaryPointerUp(event);
				currentX = (int) MotionEventCompat.getX(event,
						MotionEventCompat.findPointerIndex(event, mActivePointerId));
				currentY = (int) MotionEventCompat.getY(event,
						MotionEventCompat.findPointerIndex(event, mActivePointerId));
				break;
		}
		if (velocityTracker != null) {
	...
	public boolean onStartNestedScroll(View child, View target, int nestedScrollAxes) {
		return (nestedScrollAxes & ViewCompat.SCROLL_AXIS_VERTICAL) != 0 || (nestedScrollAxes & ViewCompat.SCROLL_AXIS_HORIZONTAL) != 0;
	}
	...
	private void flingWithNestedDispatch(int velocityX, int velocityY) {
		...
		final boolean canXFling = (scrollX > 0 || velocityX > 0) && (scrollX < getScrollXRange() || velocityX < 0);
		final boolean canYFling = (scrollY > 0 || velocityY > 0) && (scrollY < getScrollYRange() || velocityY < 0);
		if (!dispatchNestedPreFling(velocityX, velocityY)) {
			dispatchNestedFling(velocityX, velocityY, canXFling || canYFling);
		}
	}
